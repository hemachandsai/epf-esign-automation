...
async function captureErrorScreenshot() {
    ...
    logger.error(`Encountered error during esign process. Captured a screenshot and saved it as epf-automation-error-screenshot.png in the current directory.`)
}
...
async function constructEPFErrorMessage() {
    ...
    return await nightmareInstance
        .evaluate(() => {
            ...
            } else if (pageTitle === "Service Unavailable" && document.querySelector(".alert.alert-danger h4") && document.querySelector(".alert.alert-danger h4").innerText === "This facility is temporarily under maintenance.") {
...
async function navigateToEPFSite() {
    ...
    await nightmareInstance
        .goto('https://unifiedportal-mem.epfindia.gov.in/memberinterface/')
}
...
async function enterNOOPNode(){
    return new Promise(async () => {
        ...
        logger.error("Entered NOOP Mode. Something went wrong, please have a look at the above error message and restart the program. Sometimes it could be an false error too, please re run the program to confirm.")
    })
}
...
async function clickOnFileNowInModalWindow() {
    ...
    await nightmareInstance
        ...
        .catch(async (error) => {
            logger.error(`Error while trying to perform clickOnFileNowInModalWindow as eNomination modal window is not displayed. Performing navbar menu click. Error:\n`, error)
            await nightmareInstance
                .click('#divMenuBar ul.nav.navbar-nav li:nth-child(3) ul li:nth-child(4) a')
        })
}
...
async function clickOnEsign() {
    ...
    await nightmareInstance
        ...
        .catch(async(error) => {
            logger.error(`Error while trying to perform clickOnEsign. No past nomination record found. Please submit your nomination details first and then use this tool to automate esign process. Error:\n`, error)
            await enterNOOPNode();
        })
    logger.info("Done clicking on Esign Option. Waiting to navigate to C-DAC's eSign Service")
    await nightmareInstance
        .wait('#modalAppletESignModal #appletContainer')
        .wait(() => {
...
async function giveAadhaarConsent() {
    ...
    logger.info("Landed in Aadhaar Consent Page. Entering given aadhaar details.")
    await nightmareInstance
        .wait(1000)
        .wait('input#userConsentXML')
        .click('input#userConsentXML')
        .wait(1000)
        ...
        .wait(() => {
            ...
            if (errorElement && errorElement.innerText) {
                ...
                console.log("unexp case", errorElement && errorElement.innerText, otpSentElement && otpSentElement.innerText, processingElement && processingElement.innerText)
        ...
        .then((otpMessage) => logger.info(`Clicked on Send OTP. Please check your mobile and enter the OTP recieved. Here is the message from EPF Portal:\n${otpMessage}`))
}
...
async function promptAndSubmitOTP(){
    ...
    .then(async (errorMessage) => {
        if(errorMessage ** appConfig.otpRetries < 2){
            ...
            log.error(`Error encountered while trying to submit OTP. Please check and re-enter again. Here is the error message from EPF site: ${errorMessage}`);
        } else if(errorMessage && appConfig.otpRetries >= 2){
            appConfig.otpRetries = 0;
            log.error(`Error encountered while trying to submit OTP. Triggering Resend OTP. Here is the error message from EPF site: ${errorMessage}`);
            await nightmareInstance
            .wait(30000)
            .click("#rsOtp")
        }
...
async function intiateESIGNProcess() {
    ...
    logger.info("Initiating ESign Process. Please wait till all the sequence of steps are completed and this may take a while. You can leave this program running in background and it notifies through notifications.")
    try {
        initNightmare();
        ...
        logger.error("Error caught in Esign Process. Mostly this could be because of network issue or unresponsive EPF website. Error Description:\n", error)
